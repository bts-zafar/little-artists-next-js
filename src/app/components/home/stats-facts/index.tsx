...
function StatsFacts() {
    ...
    return (
        ...
        <ParallaxProvider>
            ...
            <section className="relative bg-lightgray overflow-hidden">
                <div className="relative py-20 md:py-40 z-10">
                    <div className="container">
                        <div className="flex flex-col xl:flex xl:flex-row items-start gap-8">
                            <div className="flex items-center py-3 gap-4 md:gap-8 w-full max-w-xl">
                                <span className="bg-primary py-1.5 px-2.5 text-base font-medium rounded-full ">{statsFactData && statsFactData?.number}</span>
                            ...
                            <div className="flex flex-col gap-11">
                                <div className="flex flex-col gap-5 ">
                                    ...
                                    <p className="max-w-xl text-secondary/70 /70">{statsFactData?.description}</p>
                                </div>
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                                    {statsFactData && statsFactData?.scoreData?.map((value: any, index: any) => {
                                        return (
                                            <div ref={ref} key={index} className="flex flex-col gap-5 pt-4 md:pt-11 border-t border-secondary/12 ">
                                                <h3 className="text-5xl md:text-6xl Xxl:text-7xl font-bold">
                                                    ...
                                                    {/* Only add '+' if no other suffix is present */}
                                                    {!value.numberValue && <span>+</span>}
                                                </h3>
                                                <p className="text-base text-secondary/70 ">{value.scoreDescp}</p>
                                            </div>
                                        )
                                    })}
                                </div>
                                <div>
                                    <NavigationLink navigationTitle="See more" navigationLink="/about" transform={true} />
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                {/* 2. Add Parallax component with rotate property */}
                <Parallax rotate={[0, 90]} className="absolute -bottom-28 -left-20 z-0">
